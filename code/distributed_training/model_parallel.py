...
def main_worker(rank,world_size,args):
    ...
    train_loader = torch.utils.data.DataLoader(
        trainset, batch_size=512, shuffle=True, num_workers=12)
    ...
    if rank == 0:
        ...
        model = MobileNetV2()
        model = nn.Sequential(model.conv1,model.bn1,model.layers[0:3])
        model.cuda(rank)
        ...
        for epoch in range(args.epochs):
            batch_time1, data_time1,acc1_avg,loss_avg = train_header(rank,model,optimizer,train_loader,criterion,args)
            ...
            batch_time1, val_acc1, val_loss = val_header(rank,model,val_loader,criterion,args)
            scheduler.step(scheduler.last_epoch+1)
            warmup_schduler.dampen()
            save_path = "./log/512.txt"
        ...
        model = MobileNetV2(num_classes= 10).layers[6*rank-3:6*rank+3]
        model.cuda(rank)
        optimizer = torch.optim.SGD(model.parameters(), args.lr, weight_decay= args.weight_decay,momentum=args.momentum)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
        warmup_schduler =warmup.LinearWarmup(optimizer,warmup_period= 10)
        ...
        model = MobileNetV2()
        model = nn.Sequential(model.layers[15:],model.conv2,model.bn2,Reshape1(),model.linear)
        model.cuda(rank)
        optimizer = torch.optim.SGD(model.parameters(), args.lr, weight_decay= args.weight_decay,momentum=args.momentum)
        criterion = nn.CrossEntropyLoss().cuda(rank)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
